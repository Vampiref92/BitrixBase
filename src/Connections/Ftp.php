...
class Ftp
{
    ...
    protected $localExportFolder = '/Vf92/files/export';
    ...
    public function __construct($params)
    {
        ...
        $this->log = LoggerFactory::create('ftp');
        if (!empty($params)) {
    ...
    {
        if (\file_exists($localPath)) {
            $filePath = $_SERVER['DOCUMENT_ROOT'] . '/Vf92/tmpFilesFtp/' . $additionalPath . '/' . \basename($ftpPath);
            $fp = \fopen($filePath, 'w');
            \fwrite($fp, 'put');
            \fclose($fp);
            exec(
    ...
    {
        $dirPath = $_SERVER['DOCUMENT_ROOT'] . '/Vf92/tmpFilesFtp/' . $additionalPath;
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                if (!\in_array($file, ['.', '..'])) {
                    ...
                    if (\is_file($filePath)) {
                        $timeDiff = (\time() - \filemtime($filePath));
    ...
    {
        $files = @\scandir($_SERVER['DOCUMENT_ROOT'] . '/Vf92/tmpFilesFtp/' . $additionalPath);
    ...
    {
        if (($files = @\scandir($_SERVER['DOCUMENT_ROOT'] . '/Vf92/tmpFilesFtp/' . $additionalPath))
